...
// ===== Playlist Directory (JSON) support =====
  async function loadPlaylistDirectory(input){
    try {
      let data = input;
      ...
      const arr = Array.isArray(data?.playlists) ? data.playlists : [];
      const items = arr.filter(p => p && typeof p.url === 'string' && p.url.trim())
                       .map(p => ({ name: (p.name||p.url).trim(), url: p.url.trim() }));

      if (!items.length) throw new Error('Aucune entrée playlist valide (url manquante).');

      const sel1 = document.getElementById('sourceSelect');
      const sel2 = document.getElementById('sourceSelect2');
      if (!sel1 || !sel2) return;

      // retire l'ancien groupe injecté
      sel1.querySelector('optgroup[data-jsondir]')?.remove();
      sel2.querySelector('optgroup[data-jsondir]')?.remove();

      const og1 = document.createElement('optgroup');
      og1.setAttribute('label', '📁 JSON');
      og1.setAttribute('data-jsondir','1');

      const og2 = document.createElement('optgroup');
      og2.setAttribute('label', '📁 JSON');
      og2.setAttribute('data-jsondir','1');

      for (const it of items){
        const o1 = document.createElement('option');
        o1.value = it.url;
        o1.textContent = it.name;
        og1.appendChild(o1);

        const o2 = document.createElement('option');
        o2.value = it.url;
        o2.textContent = it.name;
        og2.appendChild(o2);
      }
      ...
      // si un custom UI utilise un bouton data-playlists="..."
      document.addEventListener('click', function(e){
        const el = e.target && e.target.closest ? e.target.closest('[data-playlists]') : null;
        if (!el) return;
        const href = el.getAttribute('data-playlists');
        if (!href) return;
        e.preventDefault();
        loadPlaylistDirectory(href);
      }, true);
    } catch(err) {
      console.error(err);
      alert('Impossible de charger le répertoire JSON : ' + (err && err.message ? err.message : ''));
    }
  }

  // export global léger
  window.loadPlaylistDirectory = loadPlaylistDirectory;
...
// ===== Delegated click handler for external/unknown playlists =====
(function ensureUnmuteForExternalPlaylists(){
  document.addEventListener('click', function(e){
    const btn = e.target && e.target.closest && e.target.closest('[data-url]');
    if (!btn) return;

    // If this is one of our native lists, let the existing handlers run.
    if (btn.closest('#channelList, #channelList2, #inlineChannelList')) return;
    ...
    // Prevent default navigation (in case it's an <a>)
    e.preventDefault();

    // Use our unified loader (handles unmute + play promise)
    if (typeof window.loadSource === 'function') {
      window.loadSource(url, logo, name);
    } else if (typeof loadSource === 'function') {
      loadSource(url, logo, name);
    }

    // Close panels if present
    try { closeInlinePanel(); } catch(_) {}
    try { closePanel(); } catch(_) {}
  }, true); // capture phase to win over other handlers
// ===== JSON playlists support =====
  function __normalizeChannelFields(o){
    const _url  = o.url || o.streamUrl || o.link || '';
    const _name = o.name || o.title || o.label || o.station || '';
    const _logo = o.logo || o.icon || o.image || o.thumbnail || '';
    const _grp  = o.group || o.group_title || o.category || o.genre || '';
    if (!_url) return null;
    return {
      name: _name && _name.trim() ? _name : (typeof prettyUrl==='function' ? prettyUrl(_url) : _url),
      url:  _url,
      logo: _logo,
      group: _grp
    };
  ...
  function __extractChannelsFromJSON(data){
    const out = [];
    const push = (o) => {
      const ch = __normalizeChannelFields(o || {});
      if (ch) out.push(ch);
    };
    const visit = (node) => {
      if (!node) return;
      ...
      // common containers
      if (Array.isArray(node.channels)) return visit(node.channels);
      if (Array.isArray(node.items))    return visit(node.items);
      if (Array.isArray(node.playlists))return node.playlists.forEach(visit);
      // sometimes nested under 'playlist' or 'data'
      if (node.playlist) return visit(node.playlist);
      if (node.data)     return visit(node.data);
      // leaf-like object
      if (node.url || node.streamUrl || node.link) return push(node);
    };
    visit(data);
    // de-dup by url
    const seen = new Set();
  ...
  async function loadJSONPlaylist(url){
    try {
      ...
      const chans = __extractChannelsFromJSON(data);
      if (!Array.isArray(chans) || !chans.length) throw new Error('Aucune chaîne trouvée');
      // Remplace l'ensemble courant comme pour M3U
      channels = chans;
      renderCategories(channels, $('#categorySelect'), $('#categorySelect2'));
      renderChannels(channels, $('#channelList'), $('#channelList2'));
      updateViewportVars();
      ...
      console.error(err);
      alert('Impossible de charger la playlist JSON. ' + (err && err.message ? err.message : ''));
    }
  }

  // Délégué: tout élément portant data-json déclenche le chargement
  document.addEventListener('click', function(e){
    const el = e.target && e.target.closest ? e.target.closest('[data-json]') : null;
    if (!el) return;
    const url = el.getAttribute('data-json') || '';
    if (!url) return;
    e.preventDefault();
    try { loadJSONPlaylist(url); } catch(_) {}
    try { closeInlinePanel(); } catch(_) {}
    try { closePanel(); } catch(_) {}
  }, true);

  // export minimal
  window.loadJSONPlaylist = loadJSONPlaylist;
